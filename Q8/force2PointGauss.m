...
function f_n = force2PointGauss(xy, L, recordState, flux)
...
if recordState == 2
    ...
    y2 = N2 * xy(:, 2);
    y2_xi = N2_xi(1, :) * xy(:, 2);
    f_n = L.' * (N1.' * flux(x1, y1) * (x1_xi ^ 2 + y1_xi ^ 2) ^ 0.5 + N2.' * flux(x2, y2) * (x2_xi ^ 2 + y2_xi ^ 2) ^ 0.5);
    return
end
...
if recordState == 6
    ...
    f_n = L.' * (N1.' * flux(x1, y1) * (x1_eta ^ 2 + y1_eta ^ 2) ^ 0.5 + N2.' * flux(x2, y2) * (x2_eta ^ 2 + y2_eta ^ 2) ^ 0.5);
    return
end
