...
% File name: AbaqusClassT6.m
% This file solves a 2 demensional stead state heat transfer problem using
% finite element method with 6-node triangular element type.
% Input: [file name]

classdef AbaqusClassT6 < handle
    ...
    % Get shape functions for two-point Gaussian quadrature over the
    % region.
    % Weights are not included.
    N_a = getShapeFuncT6(0.6, 0.2, true);
    N_b = getShapeFuncT6(0.2, 0.6, true);
    N_c = getShapeFuncT6(0.2, 0.2, true);
    N_d = getShapeFuncT6(1 / 3, 1 / 3, true);    
    ...
    G_a = getShapeFuncT6(0.6, 0.2, false);
    G_b = getShapeFuncT6(0.2, 0.6, false);
    G_c = getShapeFuncT6(0.2, 0.2, false);
    G_d = getShapeFuncT6(1 / 3, 1 / 3, false);
    ...
    methods
        ...
        % Constructor
        function obj = AbaqusClassT6(fileName)
            
            % Read in data.
            strGlobalNodes = '*Node';
            strElementNodes = '*Element, type=DC2D6'; % Different than Q8.
    ...
    methods (Access = private)
        ...
        function K_el = getStiffnessMatrix(obj, xy, L)
            ...
            K_el = obj.Wa * B_a.' * obj.K * B_a * detJ_a + obj.Wb * B_b.' * obj.K * B_b * detJ_b + obj.Wc * B_c.' * obj.K * B_c * detJ_c + obj.Wd * B_d.' * obj.K * B_d * detJ_d;
            K_el = K_el / 2;
        ...
        function force_n = getNeumannBC(obj,elementNum, xy, L)
            ...
            for i = 1 : obj.tempObj.getnBCNum()
                ...
                if recordState ~= 0
                    force_n = force_n + force2PointGaussT6(xy, L, recordState, currentFlux);
                end
            end
        ...
        function force_s = getSource(obj, xy, L)
            ...
            for i = 1 : obj.tempObj.getSourceNum()
                ...
                force_s = force_s + source2PointGaussT6(obj.N_a, obj.N_b, obj.N_c, obj.N_d, detJ_a, detJ_b, detJ_c, detJ_d, L, xy, currentSource, obj.Wa, obj.Wb, obj.Wc, obj.Wd); 
            end
            return
        end
